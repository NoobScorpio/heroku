...
paymentsApi='https://api.xero.com/api.xro/2.0/Payments'
...
#     print(f"JSON RESPONSE : {json_response}")
    ...
    xero_output = open(r'data/xero_output_payments.txt', 'w+')
    xero_output.write(response.text)
    xero_output.close()
    print(f"Length of Payments : {len(json_response['Payments'])}")
    print(f"XeroRequests : COMPLETED")
XeroRequests()
from datetime import datetime
def export_csv_payments():
    invoices = open(r'data/xero_output_payments.txt', 'r').read()
    json_invoice = json.loads(invoices)
    analysis = open(r'data/xero_output_payments.csv', 'w+')
    analysis.write('Date' + ',' + 'Amount')
    analysis.write('\n')
    
    for val in json_invoice['Payments']:
...
def chart_data():
    df = pd.read_csv(r'data/xero_output_payments.csv')
...
def init():
    XeroRequests()
    export_csv_payments()
    df=pd.read_csv('data/xero_output_payments.csv')
